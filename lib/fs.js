fs = (function () {
    ...
    fs.readFile = function (filename, encoding, callback) {
        ...
        var [encoding, callback] = _checkArg(encoding, callback)
        var data = '';
        data += 'type=base64&';
        data += 'name=' + filename;
    ...
    for (var index = 0,name; name=names[index]; index++) {
        !function (name) {
                ...
                get : function () {
                    return function(){
                        ...
                        return new Promise(function(resolve, reject) {
                            fs[name].apply(fs.promises,arr.concat([function(err, data){
                                if (err) 
                                    reject(err); 
                                else 
                                    resolve(data);
...
if (typeof(exitWorker)==="undefined")exitWorker = new Worker('/lib/workerforexitserver.js');